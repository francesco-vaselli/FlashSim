...
# generating a new UPSAMPLED dataset with the same event topology and saving results in a NanoAOD-like .root file
...
# upsampling option: specify upsampling factor and whether
# we should upsample the full dataset (leave None as value) or just a section (specify int index)
UPSAMPLE_FACTOR = 25
START = None
STOP = None
...
def nbd(jet_model, muon_model, root, file_path, new_root):
    ...
    Args:
            jet_model (pytorch model): the trained NF net for jet generation
            muon_model (pythorch model): the trained NF net for muon generation
    ...
    df = (
        tree.arrays(jet_cond, library="pd", entry_start=START, entry_stop=STOP)
        .astype("float32")
        .dropna()
    )
    # main difference from 1to1 case: we need to produce an upsampled index for saving the right topology later
    numb_ev = df.index.get_level_values(0).values
    ...
    print(numb_ev, len(numb_ev))
    # main upsampling idea: concatenate multiple copies of the original df
    ...
    dfm = (
        tree.arrays(muon_cond, library="pd", entry_start=START, entry_stop=STOP)
        .astype("float32")
        .dropna()
    )
    # main difference from 1to1 case: we need to produce an upsampled index for saving the right topology later
    ...
    charges = np.reshape(
        dfm["MGenMuon_charge"].values, (len(dfm["MGenMuon_charge"].values), 1)
    )
    ...
    dfe = (
        tree.arrays(["event", "run"], library="pd", entry_start=START, entry_stop=STOP)
        .astype(np.longlong)
        .dropna()
    )
    # main difference from 1to1 case: we need to produce an upsampled index for saving the right topology later
    numb_ev = np.arange(
        0, (dfe.index.get_level_values(0).values[-1] + 1) * UPSAMPLE_FACTOR
    )
    ...
    # read electron data to df (we do not generate it, so we must copy it for each event)
    dfel = (
        tree.arrays(
            ["Electron_mvaFall17V2Iso_WP90", "Electron_eta", "Electron_pt"],
            library="pd",
            entry_start=START,
            entry_stop=STOP,
        )
    ...
    # main difference from 1to1 case: we need to produce an upsampled index for saving the right topology later
    numb_ev = dfel.index.get_level_values(0).values
    # use list comprehension to save the new event list
    # (if UPSAMPLING_FACTOR=1 GO from [0, 1, 2] to [0, 1, 2, 3, 4, 5])
    l = [numb_ev + n * (numb_ev[-1] + 1) for n in np.arange(0, UPSAMPLE_FACTOR)]
    numb_ev = np.concatenate(l, axis=0)
    # main upsampling idea: concatenate multiple copies of the original df
    dfel = pd.concat([dfel] * UPSAMPLE_FACTOR, axis=0)
    numb_sub_ev = dfel.index.get_level_values(1).values
    up_index = pd.MultiIndex.from_arrays(
        [numb_ev, numb_sub_ev], names=["event", "object"]
    )
    dfel = dfel.set_index(up_index)
    print(dfel)
    ...
    print(el_ev_index)
    events_structure_el = (
        dfel.reset_index(level=1).index.value_counts().sort_index().values
    )
    ...
    # (remember that we learned ratios of mass, pt and diff of phi, eta)
    totalj[:, 7] = totalj[:, 7] + df["GenJet_eta"].values
    totalj[:, 9] = totalj[:, 9] * df["GenJet_mass"].values
    totalj[:, 11] = totalj[:, 11] + df["GenJet_phi"].values
    # account for phi periodicity
    totalj[:, 11] = np.where(
        totalj[:, 11] < -np.pi, totalj[:, 11] + 2 * np.pi, totalj[:, 11]
    )
    totalj[:, 11] = np.where(
        totalj[:, 11] > np.pi, totalj[:, 11] - 2 * np.pi, totalj[:, 11]
    )
    ...
    # postprocessing
    # transform muons back to physical distributions (remeber we learned preprocessed distributions)
    ...
    df["MMuon_ptRatio"] = df["MMuon_ptRatio"].apply(
        lambda x: np.tan(x) / 10 + 1
    )  # np.tan((x/10)+1))
    ...
    df["MMuon_pfRelIso04_all"] = df["MMuon_pfRelIso04_all"].apply(
        lambda x: np.exp(x) - 0.00001
    )

    arr = df["MMuon_pfRelIso03_all"].values
    arr = np.where(arr <= -7.5, -11.51, arr)
    df["MMuon_pfRelIso03_all"] = arr
    df["MMuon_pfRelIso03_all"] = df["MMuon_pfRelIso03_all"].apply(
        lambda x: np.exp(x) - 0.00001
    )
    ...
    df["MMuon_pfRelIso03_chg"] = df["MMuon_pfRelIso03_chg"].apply(
        lambda x: np.exp(x) - 0.00001
    )
    ...
    totalm[:, 1] = np.where(
        totalm[:, 1] < -np.pi, totalm[:, 1] + 2 * np.pi, totalm[:, 1]
    )
    totalm[:, 1] = np.where(
        totalm[:, 1] > np.pi, totalm[:, 1] - 2 * np.pi, totalm[:, 1]
    )
    ...
    to_ttreel = dict(
        zip(
            ["Electron_mvaFall17V2Iso_WP90", "Electron_eta", "Electron_pt"], to_ttreel.T
        )
    )
    ...
    with uproot.recreate(f"{new_path}_synth_upsampled_1on25.root") as file:
        file["Events"] = {
            "Jet": to_ttreej,
            "Muon": to_ttreem,
            "Electron": to_ttreel,
