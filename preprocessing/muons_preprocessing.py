...
if __name__ == "__main__":
    ...
    # apply saturations to distribution tails
    ...
    # apply conditioning gen variables preprocessing
    ...
    # apply target reco variables preprocessing
    df["MMuon_etaMinusGen"] = df["MMuon_etaMinusGen"].apply(
    ...
    df["MMuon_pfRelIso04_all"] = df["MMuon_pfRelIso04_all"].apply(
        lambda x: np.log(x + 0.00001)
    )
    arr = df["MMuon_pfRelIso04_all"].values
    arr[arr <= -7.5] = np.random.normal(
        loc=-11.51, scale=1, size=arr[arr <= -7.5].shape
    )
    ...
    df["MMuon_isGlobal"] = df["MMuon_isGlobal"].apply(
        lambda x: x + 0.1 * np.random.normal()
    )
    df["MMuon_isPFcand"] = df["MMuon_isPFcand"].apply(
        lambda x: x + 0.1 * np.random.normal()
    )
    df["MMuon_isTracker"] = df["MMuon_isTracker"].apply(
        lambda x: x + 0.1 * np.random.normal()
    )
    df["MMuon_mediumId"] = df["MMuon_mediumId"].apply(
        lambda x: x + 0.1 * np.random.normal()
    )
    df["MMuon_softId"] = df["MMuon_softId"].apply(
        lambda x: x + 0.1 * np.random.normal()
    )
    df["MMuon_softMvaId"] = df["MMuon_softMvaId"].apply(
        lambda x: x + 0.1 * np.random.normal()
    )
    # remove any numerical error
    df = df[~df.isin([np.nan, np.inf, -np.inf]).any(1)]
    print(df)
